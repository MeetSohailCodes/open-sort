...
import RenameOptions, {
  RenameStrategy,
  RenameLabelPosition,
  RenameDatePosition,
} from "./components/RenameOptions";
...
export default function App() {
  ...
  const [selectedExtensions, setSelectedExtensions] =
    useState<ExtensionsByCategory>(EXTENSION_OPTIONS);
  const [ignoredDirs, setIgnoredDirs] =
    useState<string[]>(DEFAULT_IGNORED_DIRS);
  const [showCancelModal, setShowCancelModal] = useState(false);
  const [isSameFolder, setIsSameFolder] = useState(false);
  const [renameEnabled, setRenameEnabled] = useState(false);
  const [renameStrategy, setRenameStrategy] =
    useState<RenameStrategy>("datetime_original");
  const [renameLabel, setRenameLabel] = useState("");
  const [renameLabelPosition, setRenameLabelPosition] =
    useState<RenameLabelPosition>("prefix");
  ...
  useEffect(() => {
    try {
      ...
      if (
        parsed?.organizeMode === "category" ||
        parsed?.organizeMode === "year" ||
        parsed?.organizeMode === "month"
      ) {
      ...
      if (
        parsed?.renameLabelPosition === "prefix" ||
        parsed?.renameLabelPosition === "suffix"
      ) {
        setRenameLabelPosition(parsed.renameLabelPosition);
      }
      if (
        parsed?.renameDatePosition === "prefix" ||
        parsed?.renameDatePosition === "suffix" ||
        parsed?.renameDatePosition === "none"
      ) {
  ...
  const startOrganization = useCallback(() => {
    ...
    const connectTimeout = setTimeout(() => {
      if (ws.current && ws.current.readyState !== WebSocket.OPEN) {
        setErrorMessage(
          "Backend WebSocket connection timed out. Please restart the backend.",
        );
  ...
  const handleAddIgnoreDir = (value: string) => {
    ...
    if (!cleaned) return;
    if (ignoredDirs.some((dir) => dir.toLowerCase() === cleaned.toLowerCase()))
      return;
  ...
  const configCardClass =
    "bg-slate-700/30 rounded-2xl border border-slate-600/30 p-8 min-h-[520px]";
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="text-center mb-10">
                  <Typography
                    as="h2"
                    variant="h2"
                    className="text-3xl text-slate-50 mb-3"
                  >
                    Select Directories
                  </Typography>
                  <Typography
                  ...
                  >
                    Choose your source folder containing media files and the
                    destination for organized output.
                  </Typography>
                </div>
                ...
                <div className="bg-slate-700/30 rounded-2xl border border-slate-600/30 p-8 space-y-8">
                  <Input
                    ...
                    classNames={{
                      label:
                        "text-xs font-bold text-slate-400 tracking-wider pb-2",
                  ...
                  {isSameFolder && (
                    <div className="flex items-start gap-3 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl">
                      ...
                      <div>
                        ...
                        >
                          Source and destination are the same. Files will be
                          reorganized within this folder.
                        </Typography>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="pt-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <Typography
                        as="p"
                        variant="caption"
                        className="text-slate-400 font-bold tracking-wider"
                      >
                        Organization Type
                      </Typography>
                      <Tooltip
                        content={
                          <Typography as="span" variant="body-sm">
                            Choose how files are structured inside the
                            destination
                          </Typography>
                        }
                      >
              ...
              >
                <div className="text-center mb-10">
                  ...
                  <Typography
                    ...
                    className="text-slate-400 max-w-md mx-auto"
                  >
                    Select which file types to organize and folders to skip
                    during scanning.
                  </Typography>
                </div>
              ...
              >
                <div className="text-center mb-10">
                  ...
                  <Typography
                    ...
                    className="text-slate-400 max-w-md mx-auto"
                  >
                    Configure how files should be renamed using metadata dates
                    and custom labels.
                  </Typography>
                </div>
              ...
              >
                <div className="text-center mb-10">
                  ...
                  <Typography
                    ...
                    className="text-slate-400 max-w-md mx-auto"
                  >
                    Please review your settings before starting the organization
                    process.
                  </Typography>
                </div>
                ...
                <div className="bg-slate-700/30 rounded-2xl border border-slate-600/30 p-8 space-y-6">
                    ...
                    <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-600/30">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <Typography
                          as="span"
                          variant="caption"
                          className="text-slate-400 tracking-wider font-bold"
                        >
                          Source
                        </Typography>
                      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="bg-slate-800/50 rounded-xl p-5 border border-slate-600/30 text-center">
                      ...
                      <Typography
                        as="p"
                        variant="caption"
                        className="text-slate-400 tracking-wider mt-1"
                      >
                        File Types
                      </Typography>
                    </div>
                  ...
                  <div className="flex items-start gap-3 p-4 bg-slate-500/50 border border-slate-700 rounded-xl">
                    <Info
                      className="text-slate-100 flex-shrink-0 mt-0.5"
                      size={18}
                    />
                    <div>
                      <Typography
                      ...
                      <Typography
                        as="p"
                        variant="body-sm"
                        className="text-xs text-slate-100/80 mt-1"
                      >
                        Files will be <strong>moved</strong> (not copied) to the
                        destination. Duplicate filenames will be automatically
                        renamed. Empty source folders will be cleaned up after
                        processing. Metadata renaming runs only when enabled.
                      </Typography>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <Typography
                          as="p"
                          variant="caption"
                          className="text-slate-400 tracking-wider mb-1"
                        >
                          Current File
                        </Typography>
                        <Typography
            ...
            {step === "done" && (
              ...
              >
                ...
                >
                  Your files have been successfully organized into the
                  destination folder.
                </Typography>
                  ...
                  <div className="bg-slate-700/30 rounded-2xl border border-slate-600/30 p-8 w-full max-w-lg mb-8">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <StatCard
                        icon={<Music size={18} />}
                        label="Audio"
                        value={displayStats.audio}
                      />
                      <StatCard
                        icon={<Package size={18} />}
                        label="Archives"
                        value={displayStats.archives}
                      />
                      <StatCard
        ...
        <ModalContent className="bg-slate-800 border border-slate-700">
          ...
          <ModalBody>
            <Typography as="p" variant="body" className="text-slate-300">
              Are you sure you want to cancel the current operation? Files that
              have already been moved will remain in their new location.
            </Typography>
          </ModalBody>
          <ModalFooter>
            <Button
        ...
        <div className="max-w-6xl mx-auto px-6 py-3 flex flex-col md:flex-row items-center justify-between gap-2">
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            <Typography
              as="span"
              variant="body-sm"
              className="hidden sm:inline text-[10px] text-slate-500"
            >
              |
            </Typography>
            <a
...
function StatCard({
  ...
  return (
    ...
    >
      <div
        className={isError && value > 0 ? "text-rose-400" : "text-slate-400"}
      >
        {icon}
      </div>
      <div>
        ...
        <Typography
          as="p"
          variant="caption"
          className="text-[9px] text-slate-400 tracking-wider"
        >
          {label}
        </Typography>
      </div>
